...
namespace Quic {
  ...
  TestUtils::createQuicCryptoClientConfig(Stats::Scope& scope, Api::Api& api, const std::string& san_to_match,
  ...
  TestUtils::makeClientConnection(Event::Dispatcher& dispatcher,
    ...
    quic::ParsedQuicVersionVector supported_versions{quic::CurrentSupportedVersions()[0]};
    auto connection = std::make_unique<Quic::EnvoyQuicClientConnection>(
        conn_id, server_addr, conn_helper, alarm_factory, supported_versions, client_addr,
        dispatcher, nullptr);
...
return makeSingleHttp3Request(addr, method, url, body, host, content_type);
  ...
  TestUtils::makeSingleHttp3Request(Network::Address::InstanceConstSharedPtr& addr,
    ...
    auto crypto_config =
        TestUtils::createQuicCryptoClientConfig(mock_stats_store, api, quic_client_san_to_match, time_system); 
    auto client_addr = Network::Address::InstanceConstSharedPtr();
    Http::CodecClientProd client(
        Http::CodecClient::Type::HTTP3,
        makeClientConnection(*dispatcher, addr, client_addr , quic::QuicUtils::CreateRandomConnectionId(),
                             conn_helper, alarm_factory, quic_config, server_id, *crypto_config,
                             push_promise_index),
