...
public:
  ...
  MOCK_METHOD1(write, Api::SysCallIntResult(Network::IoHandle& io_handle));
  ...
  Api::SysCallIntResult trackWrites(Network::IoHandle& io_handle) {
    Api::SysCallIntResult result = BaseClass::write(io_handle);
    if (result.rc_ > 0) {
  ...
  Api::SysCallIntResult failWrite(Network::IoHandle& /*io_handle*/) { return {-1, EAGAIN}; }
