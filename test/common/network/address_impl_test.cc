...
void makeFdBlocking(IoHandle& io_handle) {
  const int flags = io_handle.getSocketFlag().rc_;
  ASSERT_GE(flags, 0);
  ASSERT_EQ(io_handle.setSocketFlag(flags & (~O_NONBLOCK)).rc_, 0);
}
...
void testSocketBindAndConnect(Network::Address::IpVersion ip_version, bool v6only) {
  ...
  IoHandlePtr listen_io_handle = addr_port->socket(SocketType::Stream);
  ASSERT_FALSE(listen_io_handle->isClosed()) << addr_port->asString();
  ScopedFdCloser closer1(*listen_io_handle);
  ...
  if (addr_port->ip()->version() == IpVersion::v6) {
    ...
    ASSERT_GE(
        listen_io_handle->getSocketOption(IPPROTO_IPV6, IPV6_V6ONLY, &socket_v6only, &size_int).rc_,
        0);
    EXPECT_EQ(v6only, socket_v6only);
  }

  // Bind the socket to the desired address and port.
  const Api::SysCallIntResult result = addr_port->bind(*listen_io_handle);
  ...
  ASSERT_EQ(listen_io_handle->listen(128).rc_, 0);

  auto client_connect = [](Address::InstanceConstSharedPtr addr_port) {
    // Create a client socket and connect to the server.
    IoHandlePtr client_io_handle = addr_port->socket(SocketType::Stream);
    ASSERT_FALSE(client_io_handle->isClosed()) << addr_port->asString();
    ScopedFdCloser closer2(*client_io_handle);
    ...
    makeFdBlocking(*client_io_handle);

    // Connect to the server.
    const Api::SysCallIntResult result = addr_port->connect(*client_io_handle);
    ASSERT_EQ(result.rc_, 0) << addr_port->asString() << "\nerror: " << strerror(result.errno_)
                             << "\nerrno: " << result.errno_;
  };
...
TEST(PipeInstanceTest, UnlinksExistingFile) {
  const auto bind_uds_socket = [](const std::string& path) {
    ...
    IoHandlePtr listen_io_handle = address.socket(SocketType::Stream);
    ASSERT_FALSE(listen_io_handle->isClosed()) << address.asString();
    ScopedFdCloser closer(*listen_io_handle);

    const Api::SysCallIntResult result = address.bind(*listen_io_handle);
