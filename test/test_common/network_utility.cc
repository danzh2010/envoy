...
Address::InstanceConstSharedPtr findOrCheckFreePort(Address::InstanceConstSharedPtr addr_port,
  ...
  ScopedFdCloser closer(*io_handle);
  ...
  Api::SysCallIntResult result = addr_port->bind(*io_handle);
  int err;
  ...
  if (addr_port->ip()->port() == 0) {
    return Address::addressFromIoHandle(*io_handle);
  }
  return addr_port;
}
...
bindFreeLoopbackPort(Address::IpVersion version, Address::SocketType type) {
  ...
  Api::SysCallIntResult result = addr->bind(*io_handle);
  if (0 != result.rc_) {
    io_handle->close();
  ...
  return std::make_pair(Address::addressFromIoHandle(*io_handle), std::move(io_handle));
}
